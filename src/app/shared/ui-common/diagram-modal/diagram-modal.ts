...
export class DiagramModalComponent implements AfterViewInit, OnInit {
  ...
  nameDiagram = new FormControl<string>('', [
    Validators.required,
    this.nameValidator(),
  ]);
    ...
    this.state = produce(this.state, (draft: any) => {
      ...
      if (modifiedNodeDatas && draft.selectedNodeData) {
        for (let i = 0; i < modifiedNodeDatas.length; i++) {
          ...
          const nodeKeyProperty = this.myDiagramComponent.diagram.model
  ...
  public initDiagram(): go.Diagram {
    ...
    dia.addDiagramListener('PartResized', function (e: any) {
  ...
  handleConvertDataNodeSelected() {
    ...
    if (this.state.selectedNodeData.font) {
      ...
      const sizeOption = textFontSize.find(
        (item: any) => item.code === size
      ) || {
        name: '',
        code: '',
      };
      ...
      const fontOption = textFont.find(
        (item: any) => item.code === fontFamily
      ) || {
        name: '',
        code: '',
      };
  ...
  public ngAfterViewInit() {
    if (this.observedDiagram || !this.myDiagramComponent)
      return console.warn('this.myDiagramComponent not found!');
  ...
  ngOnInit(): void {
    ...
    try {
      if (
        this.diagramData.dataNode &&
        this.diagramData.dataNode.trim() !== ''
      ) {
         console.log('Attempting to parse node data:', this.diagramData.dataNode);
      const parsedNodeData = JSON.parse(this.diagramData.dataNode);
      this.state.diagramNodeData = [...parsedNodeData];
      console.log('Loaded node data:', this.state.diagramNodeData);
      }
      ...
      if (
        this.diagramData.dataLink &&
        this.diagramData.dataLink.trim() !== ''
      ) {
        const parsedLinkData = JSON.parse(this.diagramData.dataLink);
        const convertLink = parsedLinkData.map((data: any) => {
          let points;

          if (data.points) {
            if (data.points.h) {
              points = data.points.h.map(
                (point: PointData) => new go.Point(point.S, point.P)
              );
            } else if (data.points.r) {
              points = data.points.r.map(
                (point: IPointDataP) => new go.Point(point.b, point.k)
              );
        ...
        if (
          (!this.diagramData.dataNode ||
            this.diagramData.dataNode.trim() === '') &&
          this.diagramData.image &&
          this.diagramData.image.trim() !== ''
        ) {
          console.log('Creating image node from image property');

          const imageNode = {
            id: 0,
            category: 'Image',
            source: this.diagramData.image,
            width: 200, 
            height: 150,
            text: 'Image',
            fill: Colors.while,
          ...
          this.state.diagramNodeData = [imageNode];
          console.log('Created image node:', imageNode);
        }

        this.state.diagramLinkData = [...convertLink];
        console.log('Loaded link data:', this.state.diagramLinkData);
      }
    } catch (error) {
  ...
  onClickSaveDiagram() {
    ...
    const nodeData = [...this.state.diagramNodeData];
    const linkData = [...this.state.diagramLinkData];
    ...
    const diagramData = {
      ...
      image: imageExport,
      dataNode: JSON.stringify(this.state.diagramNodeData),
      dataLink: JSON.stringify(this.state.diagramLinkData),
    };

    this.saveDiagram.emit(diagramData);
  }
  onCancel() {
    this.cancel.emit();
  }
